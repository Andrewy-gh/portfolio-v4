import { ReactNode } from 'react';
import {
  SiJavascript,
  SiPython,
  SiHtml5,
  SiCss,
  SiReact,
  SiRedux,
  SiTailwindcss,
  SiMui,
  SiVite,
  SiNodedotjs,
  SiExpress,
  SiMongodb,
  SiPostgresql,
  SiFirebase,
  SiTypescript,
  SiNextdotjs,
  SiGo,
  SiSupabase,
  SiDocker,
  SiGithubactions,
  SiOpenapiinitiative,
  SiReactquery,
  SiReactrouter,
  SiShadcnui,
  SiHono,
  SiDrizzle,
  SiCloudinary,
  SiBetterauth,
  SiTestinglibrary,
  SiVitest,
} from 'react-icons/si';
import { FaFly } from 'react-icons/fa';

interface Skill {
  id: number;
  name: string;
  icon: ReactNode;
}

export const skills: Skill[] = [
  { id: 1, name: 'Javascript', icon: <SiJavascript /> },
  { id: 2, name: 'Python', icon: <SiPython /> },
  { id: 3, name: 'HTML', icon: <SiHtml5 /> },
  { id: 4, name: 'CSS', icon: <SiCss /> },
  { id: 5, name: 'React', icon: <SiReact /> },
  { id: 6, name: 'Redux', icon: <SiRedux /> },
  { id: 7, name: 'Tailwind', icon: <SiTailwindcss /> },
  { id: 8, name: 'Mui', icon: <SiMui /> },
  { id: 9, name: 'Node JS', icon: <SiNodedotjs /> },
  { id: 10, name: 'Express', icon: <SiExpress /> },
  { id: 11, name: 'Mongo', icon: <SiMongodb /> },
  { id: 12, name: 'Postgres', icon: <SiPostgresql /> },
  { id: 13, name: 'Firebase', icon: <SiFirebase /> },
  { id: 14, name: 'Fly.io', icon: <FaFly /> },
  { id: 15, name: 'Typescript', icon: <SiTypescript /> },
  { id: 16, name: 'Next JS', icon: <SiNextdotjs /> },
  { id: 17, name: 'Vite', icon: <SiVite /> },
  { id: 18, name: 'Go', icon: <SiGo /> },
  { id: 19, name: 'Supabase', icon: <SiSupabase /> },
  { id: 20, name: 'Docker', icon: <SiDocker /> },
  { id: 21, name: 'GitHub Actions', icon: <SiGithubactions /> },
  { id: 22, name: 'OpenAPI', icon: <SiOpenapiinitiative /> },
  { id: 23, name: 'React Query', icon: <SiReactquery /> },
  { id: 24, name: 'React Router', icon: <SiReactrouter /> },
  { id: 25, name: 'shadcn/ui', icon: <SiShadcnui /> },
  { id: 26, name: 'Hono', icon: <SiHono /> },
  { id: 27, name: 'Drizzle', icon: <SiDrizzle /> },
  { id: 28, name: 'Cloudinary', icon: <SiCloudinary /> },
  { id: 29, name: 'Better Auth', icon: <SiBetterauth /> },
  { id: 30, name: 'Playwright', icon: <SiTestinglibrary /> },
  { id: 31, name: 'Vitest', icon: <SiVitest /> },
];
